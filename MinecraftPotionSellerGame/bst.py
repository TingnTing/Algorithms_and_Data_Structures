...
class BinarySearchTree(Generic[K, I]):
    ...
    def get_successor(self, current: TreeNode) -> TreeNode:
        """
            ...
            larger keys.
            :complexity: O(n), where get_minimal() is O(n)
        """
        if current.right is None:  # no element greater in subtree
            return None
        else:
            return self.get_minimal(current.right)  # get the left node

    def get_minimal(self, current: TreeNode) -> TreeNode:
        """
            Get a node having the smallest key in the current sub-tree.
            :complexity: O(n), where n is height of current (current.height)
        """
        if current.left is None:
            return current
        else:
            return self.get_minimal(current.left)  # because the smallest node is always at the left end
