...
class AVLTreeNode(TreeNode, Generic[K, I]):
    ...
    def __init__(self, key: K, item: I = None) -> None:
        ...
        self.height = 1
        self.weight = 1 #record the weight of each node

    def height_update(self)->None:
        """
            update height of a AVLTreeNode
            :complexity: O(1)
        """
    ...
    def weight_update(self)-> None:
        """
            update weight of a AVLTreeNode
            :complexity: O(1)
        """
        left, right = self.left, self.right
        if left is None and right is None:
            ...
            self.weight = left.weight +1
        else:
            self.weight = right.weight + left.weight +1
